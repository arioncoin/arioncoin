...
const char * const BITCOIN_CONF_FILENAME = "arion.conf";
const char * const BITCOIN_PID_FILENAME = "ariond.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "arion" is a composite category enabling all Arion-related debug output
            if(ptrCategory->count(string("arion"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ArionCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ArionCore
    // Mac: ~/Library/Application Support/ArionCore
    // Unix: ~/.arioncore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ArionCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ArionCore";
#else
    // Unix
    return pathRet / ".arioncore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty arion.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override arion.conf
